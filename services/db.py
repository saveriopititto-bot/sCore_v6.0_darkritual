...
class DatabaseService:
    ...
    def update_athlete_zones(self, athlete_id: int, zones: Dict[str, Any]) -> bool:
        """Aggiorna solo le zone cardiache dell'atleta"""
        try:
            self.client.table("athletes").update({"hr_zones": zones}).eq("id", athlete_id).execute()
            return True
        except Exception as e:
    ...
    def has_athlete_bests(self, athlete_id: int) -> bool:
        """Verifica se ci sono già best efforts salvati per l'atleta"""
        try:
            res = self.client.table("athlete_bests").select("best_time", count="exact").eq("athlete_id", athlete_id).limit(1).execute()
            # Se count > 0 significa che ci sono record
            return res.count is not None and res.count > 0
        except Exception as e:
            logger.error(f"Error checking athlete bests: {e}")
            return False

    def save_athlete_bests(self, athlete_id: int, bests_data: List[Dict[str, Any]]) -> bool:
        """Salva i best efforts (upsert massivo)"""
        try:
            # unique per (athlete_id, distance_type)
            self.client.table("athlete_bests").upsert(bests_data, on_conflict="athlete_id, distance_type").execute()
            return True
        except Exception as e:
            logger.error(f"Error saving athlete bests: {e}")
            return False
    ...
    def save_run(self, run_data: Dict[str, Any], athlete_id: int) -> bool:
        ...
        try:
            ...
            payload = {
                ...
                "name": run_data.get('name', 'Untitled Run'),  # NEW: activity name
                "date": run_data['Data'],             
                ...
                "is_weather_real": run_data.get('is_weather_real', False),
            ...
            logger.error(f"Payload: {payload}")  # Debug info
            return False
    ...
    def get_history(self, athlete_id: int = None) -> List[Dict[str, Any]]:
        """Carica lo storico mappando SQL Supabase -> Dati Python
        
        Args:
            athlete_id: Optional filter to get runs for specific athlete only.
                       If None, returns all runs (for admin/debugging).
        """
        try:
            ...
            for row in data:
                ...
                processed.append({
                    "id": row['id'],
                    "Data": row['date'],
                    "Moving Time": row.get('moving_time', 0),
    ...
    # --- STREAK PERSISTENTE ---
    def update_streak(self, athlete_id: int) -> None:
    ...
    # --- BASELINES MANAGEMENT (v5) ---
    def get_athlete_baseline(self, athlete_id: int, distance_label: str) -> Optional[float]:
        """
        Recupera il Best Time Adjusted (T_adj) per una specifica distanza.
        """
        try:
            res = self.client.table("athlete_baselines") \
                .select("best_time_adj") \
                .eq("athlete_id", athlete_id) \
                .eq("distance_label", distance_label) \
                .execute()
            
            return res.data[0]['best_time_adj'] if res.data else None
        except Exception as e:
            logger.error(f"Errore recupero baseline {distance_label}: {e}")
            return None

    def update_athlete_baseline(self, athlete_id: int, distance_label: str, new_time_adj: float):
        """
        Aggiorna (o crea) la baseline per una distanza specifica.
        ...
        try:
            data = {
                ...
                "best_time_adj": new_time_adj,
                "updated_at": datetime.now().isoformat()
            }
            
            # Upsert con gestione conflitti
            response = self.client.table("athlete_baselines").upsert(
                data, 
                on_conflict="athlete_id, distance_label"
            ).execute()
            
            logger.info(f"🏆 Baseline aggiornata per {distance_label}: {new_time_adj:.2f}s")
            return response.data
            
        except Exception as e:
            logger.error(f"Errore aggiornamento baseline: {e}")
            return None

    def get_weather_audit(self, limit: int = 15) -> List[Dict[str, Any]]:
        """
        Recupera dati per audit meteo: Date, Name, Temp, Hum, is_weather_real
        """
        try:
            # Recuperiamo tutto per flessibilità (o specificare colonne se schema è certo)
            # Assumiamo che le colonne 'is_weather_real' e 'name' esistano o siano in raw_data
            # Se la colonna non esiste nella select standard, Supabase potrebbe ignorarla o errorare.
            # Usiamo * e processiamo in python per sicurezza
            res = self.client.table("runs")\
                .select("*")\
                .order("date", desc=True)\
                .limit(limit)\
            ...
            for r in res.data:
                ...
                # Nome corsa: da colonna dedicated o raw o fallback
                name = r.get("name") or raw.get("name") or "Corsa senza nome"
                
                # Flag Meteo Reale
                # Se non c'è colonna, cerchiamo in raw. Default False per sicurezza audit
                is_real = r.get("is_weather_real")
                if is_real is None:
                    is_real = raw.get("is_weather_real", False)
                
                # Meteo
                # Supponiamo columns 'temperature' e 'humidity' o estraiamo da meteo_desc/raw
                temp = r.get("temperature")
                if temp is None: temp = raw.get("temperature", 20.0) # Default score app
                
                hum = r.get("humidity") 
                if hum is None: hum = raw.get("humidity", 50.0)
                ...
                data.append({
                    ...
                    "temperature": temp,
                    "humidity": hum,
                    "is_weather_real": bool(is_real)
                })
            
            return data
        except Exception as e:
            logger.error(f"Errore audit meteo: {e}")
            return []
